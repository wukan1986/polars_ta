...
def {name}({aa}) -> Expr:  # {output_names}
    \"\"\"{doc}\"\"\"
    return struct([{bb}]).map_batches(lambda xx: batches_i2_o2([xx.struct[i].to_numpy().astype(float) for i in range({dd})], {cc}))
"""
    ...
    if len(input_names) == 1 and len(output_names) == 1:
        return tpl11.format(name=name, aa=aa, bb=bb, cc=cc, dd=len(input_names), output_names=output_names, doc=doc)
    elif len(input_names) == 1 and len(output_names) > 1:
        return tpl12.format(name=name, aa=aa, bb=bb, cc=cc, dd=len(input_names), output_names=output_names, doc=doc)
    elif len(input_names) > 1 and len(output_names) == 1:
        return tpl21.format(name=name, aa=aa, bb=bb, cc=cc, dd=len(input_names), output_names=output_names, doc=doc)
    else:
        return tpl22.format(name=name, aa=aa, bb=bb, cc=cc, dd=len(input_names), output_names=output_names, doc=doc)


def codegen():
    head_v2 = """# generated by codegen_talib2.py
import talib as _ta
