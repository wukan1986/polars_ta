...
def _codegen_func_v2(name, input_names, parameters, output_names):
    tpl11 = """
def {name}({aa}) -> Expr:
    return {bb}.map_batches(lambda x: func_wrap_11(_ta.{name}, x, {cc}))

"""
    tpl12 = """
def {name}({aa}) -> Expr:
    return {bb}.map_batches(lambda x: func_wrap_mn(_ta.{name}, x, {cc}))

"""
    tpl2X = """
def {name}({aa}) -> Expr:
    return struct({bb}).map_batches(lambda x: func_wrap_mn(_ta.{name}, x, {cc}))
