...
import talib as _ta
from polars import Expr, map_batches, struct
...
def AROON(high: Expr, low: Expr, timeperiod: int = 14, ret_idx: int = 1) -> Expr:  # ['aroondown', 'aroonup']
    ...
    Outputs:
        ...
        aroonup
    """
    return struct([high, low]).map_batches(lambda xx: batches_i2_o2([xx.struct[i].to_numpy().astype(float) for i in range(2)], _ta.AROON, timeperiod, ret_idx=ret_idx))
...
def STOCH(high: Expr, low: Expr, close: Expr, fastk_period: int = 5, slowk_period: int = 3, slowk_matype: int = 0, slowd_period: int = 3, slowd_matype: int = 0, ret_idx: int = 1) -> Expr:  # ['slowk', 'slowd']
    ...
    Outputs:
        ...
        slowd
    """
    return struct([high, low, close]).map_batches(lambda xx: batches_i2_o2([xx.struct[i].to_numpy().astype(float) for i in range(3)], _ta.STOCH, fastk_period, slowk_period, slowk_matype, slowd_period, slowd_matype, ret_idx=ret_idx))
...
def STOCHF(high: Expr, low: Expr, close: Expr, fastk_period: int = 5, fastd_period: int = 3, fastd_matype: int = 0, ret_idx: int = 1) -> Expr:  # ['fastk', 'fastd']
    ...
    Outputs:
        ...
        fastd
    """
    return struct([high, low, close]).map_batches(lambda xx: batches_i2_o2([xx.struct[i].to_numpy().astype(float) for i in range(3)], _ta.STOCHF, fastk_period, fastd_period, fastd_matype, ret_idx=ret_idx))
