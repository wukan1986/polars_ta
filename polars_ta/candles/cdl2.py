...
def gap_up(open_: Expr, high: Expr, low: Expr, close: Expr) -> Expr:
    """跳空高开"""
    return low > high.shift(1)


def gap_down(open_: Expr, high: Expr, low: Expr, close: Expr) -> Expr:
    """跳空低开"""
    return high < low.shift(1)


def real_body_gap_up(open_: Expr, high: Expr, low: Expr, close: Expr) -> Expr:
    """实体跳空高开"""
    return lower_body(open_, high, low, close) > upper_body(open_, high, low, close).shift(1)


def real_body_gap_down(open_: Expr, high: Expr, low: Expr, close: Expr) -> Expr:
    """实体跳空低开"""
